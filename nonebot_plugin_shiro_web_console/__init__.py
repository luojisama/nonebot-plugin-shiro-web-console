...
_p13n_detected_provider_cache: List[PersonificationProvider] = []
_p13n_provider_cache_key: Optional[str] = None


def _detect_personification_module_names() -> List[str]:
    try:
...
def _build_personification_provider(module_name: str) -> PersonificationProvider:
    ...
    api_module = importlib.import_module(f"{module_name}.web_console_api")
    provider_info: Dict[str, Any] = {}
    info_getter = getattr(api_module, "get_provider_info", None)
    if callable(info_getter):
        raw_info = info_getter()
        if isinstance(raw_info, dict):
            provider_info = dict(raw_info)
    api_version = int(
        provider_info.get(
            "api_version",
            getattr(api_module, "WEB_CONSOLE_API_VERSION", 0),
        )
        or 0
    )
    if api_version != 1:
        raise RuntimeError(f"不支持的 web_console_api 版本: {api_version}")

    return PersonificationProvider(
        variant=str(provider_info.get("variant") or candidate["variant"]),
        module_name=str(provider_info.get("module_name") or candidate["module_name"]),
        display_name=str(provider_info.get("display_name") or candidate["display_name"]),
...
def _serialize_personification_provider(provider: PersonificationProvider) -> Dict[str, Any]:
    payload = dict(provider.provider_info) if isinstance(provider.provider_info, dict) else {}
    payload.setdefault("variant", provider.variant)
    payload.setdefault("module_name", provider.module_name)
    payload.setdefault("display_name", provider.display_name)
    payload["api_version"] = provider.api_version
    return payload


def _get_detected_personification_providers(force_refresh: bool = False) -> List[PersonificationProvider]:
    global _p13n_provider_cache
    global _p13n_detected_provider_cache
    global _p13n_provider_cache_key

    module_names = _detect_personification_module_names()
    cache_key = "|".join(module_names)
    if not module_names:
        _p13n_provider_cache = None
        _p13n_detected_provider_cache = []
        _p13n_provider_cache_key = None
        return []

    if (
        not force_refresh
        and _p13n_provider_cache_key == cache_key
    ):
        return list(_p13n_detected_provider_cache)

    providers: List[PersonificationProvider] = []
    for module_name in module_names:
        try:
            providers.append(_build_personification_provider(module_name))
        except Exception as e:
            logger.error(f"拟人插件集成初始化失败，将跳过 {module_name}: {e}")

    _p13n_detected_provider_cache = providers
    _p13n_provider_cache = providers[0] if providers else None
    _p13n_provider_cache_key = cache_key if providers else None

    if _p13n_provider_cache is not None:
        selected = _p13n_provider_cache
        detected = ", ".join(provider.module_name for provider in providers)
        logger.info(
            f"拟人插件已检测到，Web 控制台启用 provider: {selected.display_name} ({selected.module_name});"
            f" detected={detected}"
        )
...
def _get_personification_provider(force_refresh: bool = False) -> Optional[PersonificationProvider]:
    providers = _get_detected_personification_providers(force_refresh=force_refresh)
    return providers[0] if providers else None


def _with_detected_personification_providers(
    ...
    *,
    ...
    providers = _get_detected_personification_providers(force_refresh=force_refresh)
    response["detected_providers"] = [
        _serialize_personification_provider(provider) for provider in providers
    ]
    if providers and not isinstance(response.get("provider"), dict):
        response["provider"] = _serialize_personification_provider(providers[0])
    return response


def _personification_unavailable_response(*, force_refresh: bool = False) -> Dict[str, Any]:
    return _with_detected_personification_providers(
        {
            "available": False,
...
def _call_personification_api(
    provider: PersonificationProvider,
    method_name: str,
    *args: Any,
    **kwargs: Any,
) -> Dict[str, Any]:
    method = getattr(provider.api_module, method_name, None)
    if not callable(method):
        raise RuntimeError(f"{provider.module_name}.web_console_api 未提供 {method_name}")
    result = method(*args, **kwargs)
    if not isinstance(result, dict):
        raise RuntimeError(f"{provider.module_name}.web_console_api.{method_name} 必须返回 dict")
    payload = dict(result)
    payload.setdefault("provider", _serialize_personification_provider(provider))
    return payload
...
def _is_personification_stack() -> bool:
    providers = _get_detected_personification_providers()
    if not providers:
        return False
    ...
    try:
        ...
        while frame:
            ...
            if any(module_name.startswith(provider.module_name) for provider in providers):
                return True
            frame = frame.f_back
    except Exception as e:
...
if app:
    ...
    async def get_plugins():
        ...
        for p in get_loaded_plugins():
            ...
            if module_name.startswith("nonebot.plugins"):
                plugin_type = "builtin"
            elif module_name == "nonebot_plugin_personification":
                plugin_type = "store"
            elif module_name == "personification":
                plugin_type = "local"
            ...
            ver = "0.0.0"
            pkg_names_to_try = [
                "nonebot-plugin-shiro-personification" if module_name == "nonebot_plugin_personification" else module_name,
                module_name,
        ...
        for dist in distributions():
            ...
            # 推测模块名：将连字符替换为下划线
            if pkg_name == "nonebot-plugin-shiro-personification":
                module_name = "nonebot_plugin_personification"
            else:
    ...
    async def get_personification_status():
        ...
        if provider is None:
            return _personification_unavailable_response()

        try:
            return _with_detected_personification_providers(
                _call_personification_api(provider, "get_status")
            )
        except Exception as e:
    ...
    async def get_personification_config():
        ...
        try:
            return _with_detected_personification_providers(
                _call_personification_api(provider, "get_global_config_entries")
            )
        except Exception as e:
    ...
    async def update_personification_config(data: dict):
        ...
        try:
            return _with_detected_personification_providers(
                _call_personification_api(provider, "update_global_config", data or {})
            )
        except Exception as e:
    ...
    async def get_personification_stats():
        ...
        try:
            payload = _call_personification_api(provider, "get_stats")
            groups = {
                key: {
            ...
            return _with_detected_personification_providers(
                {"available": False, "groups": {}, "error": str(e)}
            )
    ...
    async def get_personification_group_config(group_id: str):
        ...
        try:
            return _with_detected_personification_providers(
                _call_personification_api(provider, "get_group_config", normalized_group_id)
            )
        except Exception as e:
